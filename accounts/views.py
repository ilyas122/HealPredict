...
def signup_patient(request):
    if request.method == 'POST':
        ...
        if username and email and name and password and password1:
            if password == password1:
                if User.objects.filter(username=username).exists():
                    ...
                    # Assuming dob is not a required field, it's removed from patient creation
...
def savepdata(request,patientusername):
  ...
  if request.method == 'POST':
    ...
    patient.objects.filter(pk=puser.patient).update(name=name)
